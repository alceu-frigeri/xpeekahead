...
Two sets of commands are defined, one to be used in a \tsobj[pkg]{expl3} package (see \ref{expl3-cmds}), and another for use in a \LaTeXe\ code régime (see \ref{2e-cmds}).
\begin{tsremark}
...
These commands aren't \tsobj[pkg]{expl3} conditionals, both \tsobj[marg]{if-true,if-false} parts are always obligatory, the \tsobj{TF} ending is for readability only and each command has a base command with a \tsobj{nn} ending.  To generate variants, use either  \tsobj{\exp_args:} or \tsobj{\cs_generate_variant:}.
   In case of the last two arguments, \tsobj{TF}, use the \tsobj{nn} base command, e.g., \tsobj{\cs_generate_variant:Nn}\tsobj{\xpeekahead_set:Nnnn {NVVV}}.
\end{tsremark}
...
\end{codesyntax}
Those are for the most simple cases, where \tsobj[code,sep=or]{\xpeekahead_cmd_peek:} will be placed at the very end of a command definition. \tsobj[marg]{regex,if-true,if-false} are the same as defined in \ref{cmd-def-expl3} and \tsobj[marg]{cmd} is any command defined using \tsobj[code,sep=or]{\xpeekahead_(g)set:}.
\end{codedescribe}
